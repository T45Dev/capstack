...
10. **Overall Dilution uses ONE fully-diluted basis for both Pre and Post.** `preFDS` and `postFDS` both equal the post-round total (`aggregate base + current round's cumulative total_shares_fds`). So a holder only moves pre→post when their own share count changes (CN conversions, proposed grants); the denominator does not grow round-over-round, and existing non-participating holders sit at Δ ≈ 0. Was: Pre % divided by the prior round's FDS (isolating that round's dilution event). Proposed/idea grants still augment only the POST numerator, never the denominator (they draw from pool capacity already inside total_shares_fds).
