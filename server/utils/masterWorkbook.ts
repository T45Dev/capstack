...
// Build the master import workbook: one tab per entity, with the exact
// headers the importer expects, a leading Instructions sheet documenting the
// controlled (non-free-text) fields, and in-cell dropdowns on those fields.
//
// `prefill` optionally seeds data rows, keyed by sheet name → an array of row
// objects keyed by the spec column `key`. This is how a Carta export is turned
// into a prefilled template the operator only has to fill the gaps in. Pass no
// prefill for a blank template.
export async function buildMasterWorkbook(
  company: { name?: string | null; slug?: string | null },
  prefill?: Record<string, Array<Record<string, any>>>,
): Promise<Buffer> {
  ...
  for (const tab of MASTER_TABS) {
    ...
    // Prefilled data rows (Carta bootstrap). Row 3 onward.
    const rows = prefill?.[tab.sheet] || []
    rows.forEach((row, ri) => {
      tab.columns.forEach((c, ci) => {
        const v = row[c.key]
        if (v !== undefined && v !== null && v !== '') ws.getCell(3 + ri, ci + 1).value = v
      })
    })
